...
def add_quotes_to_values(data_frame)->pd.DataFrame:
    try:
        for column in data_frame.columns:
            count = data_frame[column][data_frame[column] == '?'].count()
            if count != 0:
                data_frame[column] = data_frame[column].replace('?', "'?'")
        return data_frame        
...
def replace_invalid_values_with_null(data):
